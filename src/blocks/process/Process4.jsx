...
function TimelineCard({ cards, activeStep, handleStep }) {
  return (
    <Timeline sx={{ [`& .${timelineItemClasses.root}:before`]: { flex: 0, padding: 0 }, p: 0, mb: 0, mt: { xs: 4, sm: 0 } }}>
      {cards.map((card, index) => (
        <TimelineItem
          ...
          sx={{ cursor: 'pointer', color: activeStep == index ? 'primary.main' : 'inherit' }}
        >
          <TimelineSeparator>
            <TimelineDot
              sx={{
...
export default function Process4({ heading, caption, cards }) {
...
TimelineCard.propTypes = { cards: PropTypes.array, activeStep: PropTypes.number, handleStep: PropTypes.func };

Process4.propTypes = { heading: PropTypes.string, caption: PropTypes.string, cards: PropTypes.array };
